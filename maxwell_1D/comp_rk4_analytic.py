...
if __name__ == "__main__":
	...
	for i in range(0, maxiter):
		...
		if (i % 200 == 0):
			# ploting results
			plt.plot(greville, rkfoure, label='rk4', color='b')
			plt.plot(greville, efieldex, label='analytic', color='r')
			plt.legend()
			plt.title('Electric field\ndt='+str(deltaT)+' t='+str(t))
			plt.savefig('Images/ElectricField1D/rk4/compare_scheme_rk4_dt'+str(deltaT)+'_t'+str(t)+'.png')
			plt.clf()
			print("Graph saved in 'Images/ElectricField1D/rk4/compare_scheme_rk4_dt"+str(deltaT)+"_t"+str(t)+".png'")
