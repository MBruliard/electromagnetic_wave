		...
		while (t < finalT):
			...
			if (savefig):
				if (i % 100 == 0):
					...
					plt.xlabel('$\Omega$')
					plt.ylim(-1.3, 1.3)
					plt.title('Approximation of the electromagnetic fields with Euler Explicit scheme\nt='+str(t)+' dt='+str(deltaT))
					plt.savefig('Images/EulerExpl/eulerexpl_dt'+str(deltaT)+'_it'+str(int(i/100)+1)+'.png')
					print("Image saved in 'Images/EulerExpl/eulerexpl_dt"+str(deltaT)+"_it"+str(int(i/100)+1)+".png'") 
					plt.clf()
