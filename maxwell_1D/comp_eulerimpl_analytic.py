...
#		# euler implicit
		...
		if (i % 200 == 0):
			# ploting results
			plt.plot(greville, imple, label='euler implicit', color='g')
			plt.plot(greville, efieldex, label='analytic', color='r')
			plt.legend()
			plt.title('Electric field\ndt='+str(deltaT)+' t='+str(t))
			plt.savefig('Images/ElectricField1D/EulerImpl/compare_scheme_euler_impl_dt'+str(deltaT)+'_t'+str(t)+'.png')
			plt.clf()
			print("Graph saved in 'Images/ElectricField1D/EulerImpl/compare_scheme_euler_impl_dt"+str(deltaT)+"_t"+str(t)+".png'")
