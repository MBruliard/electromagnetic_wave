...
#		# rk2
		...
		if (i % 200 == 0):
			# ploting results
#			
			plt.plot(greville, rktwoe, label='rk2', color='c')
			plt.plot(greville, efieldex, label='analytic', color='r')
			plt.legend()
			plt.title('Electric field\ndt='+str(deltaT)+' t='+str(t))
			plt.savefig('Images/ElectricField1D/rk2/compare_scheme_rk2_dt'+str(deltaT)+'_t'+str(t)+'.png')
			plt.clf()
			print("Graph saved in 'Images/ElectricField1D/rk2/compare_scheme_rk2_dt"+str(deltaT)+"_t"+str(t)+".png'")
