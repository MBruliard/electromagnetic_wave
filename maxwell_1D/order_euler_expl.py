...
#		explb = startB
		...
		expl = EulerExplMaxwell1D(V, W, startE, startB,  dt, final_time, epsilon0, mu0, dirichlet=True)
		exple = expl.efield()
		explb = expl.bfield()
		...
		quad_values_E = quad_values_from_vector(V, exple)
...
#	plt.plot(deltaT, order3, label='order 3', color='c')
	...
	plt.legend()
	plt.title('Error curve of electrical field with Euler Explicit\nB-Splines degree: '+str(degree))
